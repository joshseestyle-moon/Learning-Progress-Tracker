...
function levelCard(s) {
  ...
  return `
    <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.5rem;">
      ...
      <div>
        <div style="font-weight:800;font-size:1.15rem;">Lv.${s.level} ${t(s.title_key)}</div>
        <div class="text-xs text-muted">${t('xp.totalLabel', { n: s.total_xp })}</div>
      </div>
    </div>
    <div style="height:9px;border-radius:5px;background:var(--bg3,rgba(128,128,128,.2));overflow:hidden;margin-bottom:.3rem;">
      <div style="width:${pct}%;height:100%;border-radius:5px;background:#fbbf24;"></div>
    </div>
    ${nextLine}
    ${combo}`;
}

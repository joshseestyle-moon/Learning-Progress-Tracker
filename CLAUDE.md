...
- **下一輪（v3.8）待實作**：行事曆↔作業清單連動，方向＝「把行事曆的 assignments 正名為『事件』＋ 作業清單(daily_tasks) 單向顯示到行事曆」。完整規劃 `docs\plan-calendar-tasks.md`（Fable 規劃、待 Opus）。**狀態：已規劃、尚未確認動工**——開工前依全域不變量 1 先問。背景：資料調查發現 assignments 被兩個孩子當「事件」用（非作業），真作業都在 daily_tasks，故不合併。（已順手修正邦正 assignment id2 打錯的 2126→2026 年 due_date。）

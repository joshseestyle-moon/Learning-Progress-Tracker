...
#### 成績紀錄（#grades）
- 成績趨勢折線圖，可依科目篩選
- 記錄表格：日期、科目、考試名稱、得分/滿分、百分比（顏色區分高低）
- 新增/編輯/刪除成績
- 新增/編輯表單中有「從考試倒數選擇（選填）」下拉選單，選擇後自動填入考試名稱、日期、科目；也可略過選單直接手動輸入；選擇的考試記錄會以 `exam_id` FK 與成績連結

#### 課程資訊（#subjects）
- **左欄**：科目列表，可新增（選顏色）、編輯、刪除
- **右欄**：點選科目後顯示章節列表，可新增、改名、刪除、調整上下順序

#### 列印週計畫（#print）
- 產生可列印的 A4 週計畫頁面，適合列印或儲存為 PDF
- **頁首**：學習週計畫標題、使用者名稱、列印日期、本週日期範圍
- **本週課表**：僅顯示有排課的星期與節次（壓縮格式）
- **近期考試**：未完成的考試列表，含天數倒數（紅/黃/綠）與科目讀書進度條（預習/複習比例）
- **本週讀書計畫**：本週排定日期的預習/複習項目表格（日期、科目、章節、類型、完成狀態）
- **各科讀書進度**：以科目分組的章節進度表，含預習（✓/○）、每次複習（✓/○）與備註欄
- **頁尾**：系統名稱與使用者・日期
- 使用 `@media print` 隱藏側邊欄與控制按鈕，`@page { size: A4 portrait; }` 設定紙張尺寸
